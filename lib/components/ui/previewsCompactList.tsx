...
const getFileName = (item: AttachmentItem, fileType: ContentType): string =>
  item.fileName ||
  `${getAttachmentDownloadUrl(item)?.split('/').pop() ?? 'file'}.${item.contentType?.split('/').pop() || (fileType === 'video' ? 'mp4' : fileType === 'audio' ? 'mp3' : fileType === 'word' ? 'docx' : fileType === 'excel' ? 'xlsx' : fileType === 'archive' ? 'zip' : 'pdf')}`;
...
export const PreviewsCompactList = ({
  ...
  const previewableItems = validData.filter(
    item =>
      ...
      Boolean(getAttachmentPreviewUrl(item)),
  );
  ...
  useEffect(() => {
    const previewable = filterValidAttachmentItems(data).filter(
      item =>
        PREVIEW_TYPES.includes(getAttachmentContentType(item)) &&
        Boolean(getAttachmentPreviewUrl(item)),
    );
  ...
  return (
    ...
    >
      {validData.map((item, i) => {
        ...
        const sizeLabel = size != null ? `${bytesToMb(size)} ${t('mb')}` : '\u2014';
        ...
        const removeBtn = (onRemove || item.onRemove) && (
        ...
        if (isDownloadable) {
          const thumbDownloadable = item.onDownload ? (
            <button
              type='button'
              onClick={() => {
                void handleDownload();
        ...
        const previewThumbInner =
          fileType === 'image' ? (
            previewUrl ? (
              ...
              <FileIcon className='size-6 shrink-0 text-icon-fade-contrast' aria-hidden />
            )
          ) : fileType === 'video' ? (
          ...
          <div key={`${String(item.id)}-${i}`} className={cardSurfaceClass()}>
            ...
            >
              ...
              >
                ...
                <div
                  className={cn(thumbBoxClass(), fileType !== 'image' && 'text-icon-fade-contrast')}
                >
                  {previewThumbInner}
                </div>
              </button>
