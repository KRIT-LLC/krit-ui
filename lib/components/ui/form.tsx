...
const FormItem = React.forwardRef<HTMLDivElement, FormItemProps>(({ className, ...props }, ref) => {
  ...
  return (
    <FormItemContext.Provider value={{ id }}>
      <div
        ref={ref}
        className={cn('flex text-sm', props.horizontal ? 'space-x-3 items-center' : 'space-y-2 flex-col', className)}
        {...props}
      />
...
const FormMessage = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, children, ...props }, ref) => {
    ...
    return (
      <p ref={ref} id={formMessageId} className={cn('text-xs text-foreground-error blur-none', className)} {...props}>
        {body}
      </p>
    );
  }
