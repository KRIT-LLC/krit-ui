...
export const buttonVariants = cva(
  'tracking-[0.15px] inline-flex select-none gap-1 items-center justify-center whitespace-nowrap rounded-lg text-sm ring-offset-background transition-colors focus-visible:outline-none disabled:pointer-events-none disabled:opacity-80',
  {
    variants: {
      variant: {
        ...
        'theme-filled':
          'bg-background-theme text-foreground-on-contrast hover:bg-background-theme-hover disabled:bg-background-contrast-fade-disabled disabled:text-foreground-primary-disabled',
        // Соответствует старым вариантам с красным фоном: destructive, destructive-primary
        'warning-filled':
          'bg-background-error text-foreground-on-contrast hover:bg-background-error-hover disabled:bg-background-contrast-fade-disabled disabled:text-foreground-primary-disabled',
        'nav-item':
